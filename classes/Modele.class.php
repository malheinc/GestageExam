...
abstract class Modele {
    ...
    function getAllListeEntreprises() {
        $pdo = $this->connecter();
        // Requête textuelle
    ...
    function insertStage($tabValeurs) {
        $pdo = $this->connecter();
        $query = "INSERT INTO " . $this->table ."(NUM_STAGE, IDETUDIANT, IDORGANISATION, IDMAITRESTAGE, DATEDEBUT, DATEFIN, DATEVISITESTAGE, VILLE, DIVERS, BILANTRAVAUX, RESSOURCESOUTILS, COMMENTAIRES)"." VALUES ( null";
        // Pour chaque valeur à ajouter dans l'enregistrement, insérer un ?
        for ($i = 0; $i < count($tabValeurs); $i++) {
            $query.= ",'".$tabValeurs[$i]."'";
        }
